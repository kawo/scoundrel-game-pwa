 ...
 *
 * Some strings carry a little markup (<b>, <kbd>, <code>); components render
 * those through <Html> in components/Text.tsx. They are our own strings only,
 * never user input.
 *
 ...
 * one name in config.ts, not editing two files in lockstep.
 *
 * French typography: the non-breaking spaces required before : ; ! ? are
 * written as ${NB} so they are visible in the source rather than being
 * invisible characters someone deletes by accident.
 *
 * The current language is module state with a subscribe function, so React
 * reads it through useSyncExternalStore (see useLang in hooks.ts).
 */
...
const TABLES: Record<Lang, Table> = { en, fr };

/** For the translation parity tests only — the game has no reason to reach these. */
export const __tables = { en, fr, frData };
...
export function detect(): Lang {
  ...
  const wanted = (nav?.languages && nav.languages.length)
    ? nav.languages
    : [nav?.language || 'en'];
...
function fill(text: string, params?: Params | null): string {
...
let lang: Lang = (() => {
  try {
    ...
    return isLang(saved) ? saved : detect();
  } catch {
    return detect();
  }
})();
...
/**
 * Translate a UI key.
 * @param params `{n}` placeholders; `n` also selects the plural
 */
export function t(key: string, params?: Params | null): string {
 ...
 * @param key e.g. 'card.S14'
 * @param english the text to use when there is no translation
 */
export function td(key: string, english: string, params?: Params | null): string {
  if (lang !== 'en') {
...
export function setLang(next: string): boolean {
  if (!isLang(next) || next === lang) return false;
  lang = next;
  try {
...
export function onChange(fn: (lang: Lang) => void): () => void {
  listeners.add(fn);
  return () => { listeners.delete(fn); };
}

export const getLang = (): Lang => lang;
export const locale = (): string => (LANGS[lang] || LANGS.en).locale;
