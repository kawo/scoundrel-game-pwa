 ...
 *   stats   the lifetime record (see stats.ts)
 ...
 */
import type { StatsRecord } from './stats.ts';
import type { GameState, Outcome } from './types.ts';

const KEY = 'scoundrel:achievements:v1';

export type AchievementKind = 'survival' | 'blade' | 'daring' | 'mastery' | 'oddity';

export type AchievementEvent =
  | ({ type: 'resolve' } & Partial<Outcome>)
  | { type: 'avoid' | 'end' | 'view' };

export interface AchievementContext {
  stats: StatsRecord;
  /** Partial because the trophy case can be opened before a game exists. */
  state: Partial<GameState>;
  event: AchievementEvent & Partial<Outcome>;
}
...
export interface AchievementView {
  ...
  at: number | null;
  unlocked: boolean;
  bar: { current: number; goal: number } | null;
}

/* kind drives the icon: survival, blade, daring, mastery, oddity */
export const LIST: Achievement[] = [
  {
  ...
  },
  {
    ...
    test: (c) => c.event.type === 'resolve' && !!c.event.bareHanded
      && !!c.event.card && c.event.card.rank >= 11 && (c.state.health ?? 0) > 0,
  },
  {
...
let unlocked: Map<string, number> | null = null;
...
function load(): Map<string, number> {
  ...
  try {
    ...
    for (const [id, at] of Object.entries(raw)) {
      if (BY_ID.has(id)) unlocked.set(id, at as number);
    }
 ...
 * @returns the achievements newly unlocked by this event
 */
export function check(context: AchievementContext): Achievement[] {
  const seen = load();
...
export function all(context: AchievementContext): AchievementView[] {
  const seen = load();
  return LIST.map((a) => {
    let bar: AchievementView['bar'] = null;
...
export const count = (): { unlocked: number; total: number } =>
  ({ unlocked: load().size, total: LIST.length });
