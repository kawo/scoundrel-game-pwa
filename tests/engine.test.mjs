 ...
 *
 *   npm test                 quick run
 *   npm test -- --full       adds the beam-search winnability check
 *
 * Runs the real game modules from src/game/ straight under Node — Node 24
 * strips the TypeScript types itself, so there is still no build step and no
 * test framework. Only the pure modules are loaded; nothing here needs React
 * or a DOM.
 */
...
// Installed before the modules load: i18n reads the saved language at import.
Object.defineProperty(globalThis, 'localStorage', {
  value: makeStorage(), configurable: true, writable: true,
});
Object.defineProperty(globalThis, 'navigator', {
  value: { languages: ['en'] }, configurable: true, writable: true,
});

const C = await import('../src/game/config.ts');
const RNG = await import('../src/game/rng.ts');
const E = await import('../src/game/engine.ts');
const Prefs = await import('../src/game/prefs.ts');
const Stats = await import('../src/game/stats.ts');
const Ach = await import('../src/game/achievements.ts');
const I18n = await import('../src/game/i18n.ts');
const { en } = I18n.__tables;
...
group('Translations');
{
  const { en, fr, frData } = I18n.__tables;
